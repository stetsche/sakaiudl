...
(function(factory) {
    ...
    Plugin.prototype = {
        ...
        init: function() {
            ...
            $('.okayNav__menu-toggle').addClass('toggle-style');
        },
        ...
        recalcNav: function() {
        	...
        	if ($('.Mrphs-topHeader').outerWidth(true)<=self.options.mediaQuery) return;
			...
			if (nav_full_width > collapse_width + self.last_visible_child_width) {
                self._expandNavItem();
            }
        ...
        _collapseNavItem: function() {
            ...
            var $last_child = self.nav_visible.map(function() { return $(this).children('li:not(.'+self.options.notMoveClass+')').get(-1); });
...
document.addEventListener("DOMContentLoaded", function(event) {
    if( $PBJQ('.Mrphs-sitesNav__menuitem--myworkspace.is-selected').length > 0 ){
        $PBJQ('.Mrphs-sitesNav__menuitem--myworkspace').addClass('is-opened');
    }
    //$PBJQ('.siteNavWrap').css('width', $PBJQ('#Mrphs-sites-nav').outerWidth(true) + 'px' ) ;
